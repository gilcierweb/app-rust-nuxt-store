...
use crate::models::_entities::{orders, payment_gateways, payment_methods, payments, users};
use crate::models::payment_gateway_status::{PaymentAttemptStatus, PaymentMethodType};
...
fn payment_method_type_label(method_type: i16) -> String {
    match PaymentMethodType::from_i16(method_type) {
        Some(PaymentMethodType::Card) => "Cartao de Credito".to_string(),
        Some(PaymentMethodType::Wallet) => "Carteira Digital".to_string(),
        Some(PaymentMethodType::Pix) => "Pix".to_string(),
        Some(PaymentMethodType::Boleto) => "Boleto Bancario".to_string(),
        Some(PaymentMethodType::BankTransfer) => "Transferencia Bancaria".to_string(),
        Some(PaymentMethodType::Manual) => "Manual".to_string(),
        Some(PaymentMethodType::BuyNowPayLater) => "Pague Depois".to_string(),
        None => "N/A".to_string(),
    }
}

pub async fn load_receipt_data(
...
pub fn generate_receipt_pdf(data: &ReceiptData) -> Result<Vec<u8>> {
    ...
    let payment_date = data
        .payment
        .processed_at
        .map(|dt| format_datetime(dt.and_utc().fixed_offset()))
        .unwrap_or_else(|| "N/A".to_string());
    let status = payment_status_label(data.payment.status);

    let right_x = MARGIN_LEFT_MM + content_width - 60.0;
    ops.extend(text_op(
        &format!("Receipt: {payment_number}"),
        10.0,
        right_x,
        y,
        &font_regular,
    ...
    if let Some(ref ext_id) = data.payment.external_payment_id {
        ops.extend(text_op("External ID:", 10.0, label_x, y, &font_bold));
    ...
    if let Some(ref pm) = data.payment_method {
        ...
        ops.extend(text_op("Type:", 10.0, label_x, y, &font_bold));
        ops.extend(text_op(
            &payment_method_type_label(pm.method_type),
            10.0,
            value_x,
            y,
            &font_regular,
    ...
    let amount = data.payment.amount.unwrap_or(Decimal::ZERO);
    let currency = data
        .payment
        .currency
        .as_deref()
    ...
    ops.extend(text_op("Amount:", 10.0, label_x, y, &font_bold));
    ops.extend(text_op(
    ...
    ops.extend(text_op(currency, 10.0, value_x, y, &font_regular));
    y -= 20.0;
    ...
    y -= 4.0;
    ops.extend(text_op(
        &format!("{STORE_EMAIL} | This is a system-generated receipt."),
        8.0,
        MARGIN_LEFT_MM,
        y,
        &font_regular,
