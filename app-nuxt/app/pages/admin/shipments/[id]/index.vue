...
const { pending, data: shipment, error, refresh } = useFetch<Shipment>(`${config.public.baseURL}/api/shipments/${route.params.id}`)
...
const deleteShipment = async () => {
  ...
  if (confirm(`Delete shipment #${shipment.value.id}?`)) {
    try {
      await $fetch(`${config.public.baseURL}/api/shipments/${shipment.value.id}`, { method: 'DELETE' })
