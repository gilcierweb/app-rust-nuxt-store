<template>
  <div>
    ...
    <div v-else-if="order" class="mx-auto max-w-2xl py-10 text-center">
      ...
      <div class="rounded-box border p-6 text-left">
        ...
        <div class="flex justify-between py-2">
          <span class="text-base-content/60">{{ t('order.status.pending') }}</span>
...
const { data: order, pending, error } = await useFetch<Order>(
